...
//
	LBM_P = LBMbuffer;
	if ( *(long *)LBMbuffer != LittleLong(FORMID) )
	...
	formtype = LittleLong(*(long *)LBM_P);
...
//
	...
	while (LBM_P < LBMEND_P)
	{
		...
		case BODYID:
			...
			pic_p = picbuffer = SafeMalloc (320*200);
			if (bmhd.w < 320 || bmhd.h < 200)
				memset (picbuffer,0,320*200);
			if (formtype == PBMID)
			{
			//
			// unpack PBM
			//
				for (y=0 ; y<bmhd.h ; y++, pic_p += 320)
				{
